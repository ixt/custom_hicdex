...
async def on_retract_bid(
    ...
    bid = await models.Bid.filter(id=int(retract_bid.parameter.__root__), platform='bid').get_or_none()
    if bid is not None:
        ...
        ctx.logger.info('bid `%s` NOT FOUND', retract_bid.parameter.__root__)
