...
class HasuraGateway(HTTPGateway):
    ...
    async def configure(self) -> None:
        ...
        self._logger.info(
            'Adding %s generated and user-defined queries', len(query_collections_metadata))
        ...
        if self._hasura_config.rest:
            self._logger.info('Adding %s REST endpoints',
                              len(query_collections_metadata))
    ...
    async def _hasura_request(self, endpoint: str, json: Dict[str, Any]) -> Dict[str, Any]:
        self._logger.debug('Sending `%s` request: %s',
                           endpoint, dump_json(json))
    ...
    async def _healthcheck(self) -> None:
        ...
        for _ in range(timeout):
            ...
            raise HasuraError(
                f'Hasura instance not responding for {timeout} seconds')
    ...
    async def _get_views(self) -> List[str]:
        ...
        self._logger.info(
            'Found %s regular and materialized views', len(views))
        return views
    ...
    async def _generate_source_tables_metadata(self) -> List[Dict[str, Any]]:
        ...
        for app, model in iter_models(self._package):
            table_name = model._meta.db_table or pascal_to_snake(
                model.__name__)
            ...
            for field in model._meta.fields_map.values():
                if isinstance(field, fields.relational.ForeignKeyFieldInstance):
                    if not isinstance(field.related_name, str):
                        raise HasuraError(
                            f'`related_name` of `{field}` must be set')
    ...
    async def _generate_query_collections_metadata(self) -> List[Dict[str, Any]]:
        ...
        for _, model in iter_models(self._package):
            ...
            for field_name, field in model._meta.fields_map.items():
                ...
                raise RuntimeError(
                    f'Table `{table_name}` has no primary key. How is that possible?')

            fields = await self._get_fields(table_name)
            queries.append(self._format_rest_query(
                table_name, table_name, filter, fields))
    ...
    async def _get_fields(self, name: str = 'query_root') -> List[Field]:
        ...
        try:
            ...
            alternative_name = ''.join(
                [f"_{w}" if w.isnumeric() else w for w in re.split(r'(\d+)', name)])
    ...
    def _format_rest_query(self, name: str, table: str, filter: str, fields: Iterable[Field]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationError(
                f'Table `{table}` has no column `{filter}`') from e
