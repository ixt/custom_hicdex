...
class IndexDispatcher:
    ...
    async def run(
        ...
        while not self._stopped:
            tasks: List[Awaitable] = [index.process()
        ...
        for contract in contracts:
            if contract.name not in self._ctx.config.contracts:
                contract_config = ContractConfig(
                    address=contract.address, typename=contract.typename)
    ...
    async def _load_index_states(self) -> None:
        ...
        for index_state in index_states:
                ...
                if new_hash != index_state.config_hash:
                    ...
                    if old_hash == index_state.config_hash:
                        self._logger.warning(
                            'Updating config hash of index `%s`', name)
            ...
            # NOTE: Index config is missing, possibly just commented-out
                ...
                self._logger.warning(
                    'Index `%s` was removed from config, ignoring', name)
    ...
    async def _on_rollback(self, datasource: TzktDatasource, from_level: int, to_level: int) -> None:
        ...
        self._logger.warning(
            'Datasource `%s` rolled back: %s -> %s', datasource.name, from_level, to_level)
        ...
        if zero_level_rollback:
            ...
            matching_indexes = tuple(
                i for i in self._indexes.values() if i.datasource == datasource)
            matching_operation_indexes = tuple(
                i for i in matching_indexes if isinstance(i, OperationIndex))
            self._logger.info(
            ...
            all_indexes_are_operation = len(
                matching_indexes) == len(matching_operation_indexes)
...
class DipDup:
    ...
    def __init__(self, config: DipDupConfig) -> None:
        ...
        self._codegen = DipDupCodeGenerator(
            self._config, self._datasources_by_config)
    ...
    async def _set_up_database(self, stack: AsyncExitStack) -> None:
        ...
        timeout = self._config.database.connection_timeout if isinstance(
            self._config.database, PostgresDatabaseConfig) else None
    ...
    async def _set_up_hasura(self, stack: AsyncExitStack, tasks: Set[Task]) -> None:
        ...
        hasura_gateway = HasuraGateway(
            self._config.package, self._config.hasura, self._config.database)
    ...
    async def _set_up_index_dispatcher(
        ...
        tasks.add(create_task(index_dispatcher.run(
            spawn_datasources_event, start_scheduler_event)))
