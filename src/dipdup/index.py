...
OperationHandlerArgumentT = Optional[Union[Transaction,
                                           Origination, OperationData]]
MatchedOperationsT = Tuple[OperationSubgroup,
                           OperationHandlerConfig, Deque[OperationHandlerArgumentT]]
...
class Index:
    ...
    def __init__(self, ctx: DipDupContext, config: ResolvedIndexConfigT, datasource: TzktDatasource) -> None:
        ...
        self._logger = FormattedLogger(
            'dipdup.index', fmt=f'{config.name}: ' + '{}')
    ...
    async def process(self) -> None:
        ...
        if self._datasource.sync_level is None:
            raise RuntimeError(
                'Call `set_sync_level` before starting IndexDispatcher')
    ...
    async def _enter_sync_state(self, last_level: int) -> Optional[int]:
        ...
        if first_level > last_level:
            raise RuntimeError(
                f'Attempt to synchronize index from level {first_level} to level {last_level}')
    ...
    def _extract_level(self, message: Union[Tuple[OperationData, ...], Tuple[BigMapData, ...]]) -> int:
        ...
        if len(batch_levels) != 1:
            raise RuntimeError(
                f'Items in operation/big_map batch have different levels: {batch_levels}')
...
class OperationIndex(Index):
    ...
    async def _single_level_rollback(self, level: int) -> None:
        ...
        if state_level < level:
            self._logger.info(
                'Index level is lower than rollback level, ignoring: %s < %s', state_level, level)
        elif state_level == level:
            self._logger.info(
                'Single level rollback, next block will be processed partially')
            self._rollback_level = level
        else:
            raise RuntimeError(
                f'Index level is higher than rollback level: {state_level} > {level}')
    ...
    async def _process_queue(self) -> None:
        ...
        while self._queue:
            ...
            if isinstance(message, SingleLevelRollback):
                self._logger.info(
                    'Processing rollback realtime message, %s left in queue', len(self._queue))
                await self._single_level_rollback(message.level)
            else:
                self._logger.info(
                    'Processing operations realtime message, %s left in queue', len(self._queue))
    ...
    async def _synchronize(self, last_level: int, cache: bool = False) -> None:
        ...
        self._logger.info(
            'Fetching operations from level %s to %s', first_level, last_level)
    ...
    async def _process_level_operations(self, operations: Tuple[OperationData, ...]) -> None:
        ...
        if self._rollback_level:
            ...
            if len(set(levels.values())) != 1:
                ...
                raise RuntimeError(
                    f'Index is in a rollback state, but received operation batch with different levels: {levels_repr}')

            self._logger.info(
                'Rolling back to previous level, verifying processed operations')
            ...
            self._logger.info('Comparing hashes: %s new, %s missing', len(
                new_hashes), len(missing_hashes))
            if missing_hashes:
                self._logger.info(
                    'Rollback - Some operations are backtracked: %s', ', '.join(missing_hashes))
                # await self._ctx.reindex(ReindexingReason.ROLLBACK)

            self._rollback_level = None
            self._head_hashes = set()
            operations = tuple(
                op for op in operations if op.hash in new_hashes)

        # NOTE: le operator because it could be a single level rollback
        elif level < self.state.level:
            raise RuntimeError(
                f'Level of operation batch must be higher than index state level: {level} < {self.state.level}')

        self._logger.info(
            'Processing %s operations of level %s', len(operations), level)
    ...
    async def _match_operations(self, operations: Iterable[OperationData]) -> Deque[MatchedOperationsT]:
        ...
        operation_subgroups: Dict[OperationSubgroup,
                                  Deque[OperationData]] = defaultdict(deque)
            ...
            for handler_config in self._config.handlers:
                ...
                while operation_idx < len(operations):
                    ...
                    if pattern_idx == len(handler_config.pattern):
                        self._logger.info(
                            '%s: `%s` handler matched!', operation_subgroup.hash, handler_config.callback)

                        args = await self._prepare_handler_args(handler_config, matched_operations)
                        matched_subgroups.append(
                            (operation_subgroup, handler_config, args))
    ...
    async def _prepare_handler_args(
        ...
        for pattern_config, operation in zip(handler_config.pattern, matched_operations):
            if operation is None:
                ...
                try:
                    parameter = parameter_type.parse_obj(
                        operation.parameter_json) if parameter_type else None
                except ValidationError as e:
                    raise InvalidDataError(
                        parameter_type, operation.parameter_json, operation) from e
        ...
        for handler_config in self._config.handlers:
            for pattern_config in handler_config.pattern:
                if isinstance(pattern_config, OperationHandlerOriginationPatternConfig):
                    if pattern_config.originated_contract:
                        addresses.add(
                            pattern_config.originated_contract_config.address)
                    if pattern_config.source:
    ...
    async def _get_contract_hashes(self, address: str) -> Tuple[int, int]:
        if address not in self._contract_hashes:
            ...
            self._contract_hashes[address] = (
                summary['codeHash'], summary['typeHash'])
...
class BigMapIndex(Index):
    ...
    async def _synchronize(self, last_level: int, cache: bool = False) -> None:
        ...
        self._logger.info(
            'Fetching big map diffs from level %s to %s', first_level, last_level)
    ...
    async def _process_level_big_maps(self, big_maps: Tuple[BigMapData, ...]):
        ...
        if level <= self.state.level:
            raise RuntimeError(
                f'Level of big map batch must be higher than index state level: {level} <= {self.state.level}')

        self._logger.info(
            'Processing %s big map diffs of level %s', len(big_maps), level)
    ...
    async def _prepare_handler_args(
        ...
        self._logger.info('%s: `%s` handler matched!',
                          matched_big_map.operation_id, handler_config.callback)
        if not handler_config.parent:
            raise ConfigInitializationException
        ...
        if matched_big_map.action.has_key:
            ...
            try:
                ...
                raise InvalidDataError(
                    key_type, matched_big_map.key, matched_big_map) from e
        else:
            key = None
        ...
        if matched_big_map.action.has_value:
            ...
            try:
                ...
                raise InvalidDataError(
                    value_type, matched_big_map.key, matched_big_map) from e
        else:
            value = None
...
class HeadIndex(Index):
    ...
    async def _synchronize(self, last_level: int, cache: bool = False) -> None:
        self._logger.info(
            'Setting index level to %s and moving on', last_level)
        await self.state.update_status(status=IndexStatus.REALTIME, level=last_level)
    ...
    async def _process_queue(self) -> None:
        while self._queue:
            ...
            self._logger.info(
                'Processing head realtime message, %s left in queue', len(self._queue))
