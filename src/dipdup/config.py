        ...
        for table in v:
            if table.startswith('dipdup'):
                raise ConfigurationError(
                    'Tables with `dipdup` prefix can\'t be immune')
        return v
...
class TzktDatasourceConfig(NameMixin):
    ...
    def __post_init_post_parse__(self) -> None:
        ...
        if not (parsed_url.scheme and parsed_url.netloc):
            raise ConfigurationError(
                f'`{self.url}` is not a valid datasource URL')
...
DatasourceConfigT = Union[TzktDatasourceConfig,
                          BcdDatasourceConfig, CoinbaseDatasourceConfig]
...
class ParameterTypeMixin:
    ...
    def initialize_parameter_cls(self, package: str, typename: str, entrypoint: str) -> None:
        _logger.debug(
            'Registering parameter type for entrypoint `%s`', entrypoint)
        entrypoint = entrypoint.lstrip('_')
...
OperationHandlerPatternConfigT = Union[OperationHandlerOriginationPatternConfig,
...
IndexConfigT = Union[OperationIndexConfig,
                     BigMapIndexConfig, HeadIndexConfig, IndexTemplateConfig]
ResolvedIndexConfigT = Union[OperationIndexConfig,
                             BigMapIndexConfig, HeadIndexConfig]
ContractIndexConfigT = Union[OperationIndexConfig, BigMapIndexConfig]
HandlerPatternConfigT = Union[OperationHandlerOriginationPatternConfig,
                              OperationHandlerTransactionPatternConfig]
    ...
    def valid_source(cls, v):
        if v != 'default':
            raise NotImplementedError(
                'Multiple Hasura sources are not supported at the moment')
...
class JobConfig(NameMixin):
    ...
    def __post_init_post_parse__(self):
        if int(bool(self.crontab)) + int(bool(self.interval)) != 1:
            raise ConfigurationError(
                'Either `interval` or `crontab` field must be specified')
        NameMixin.__post_init_post_parse__(self)
...
class DipDupConfig:
    ...
    database: Union[SqliteDatabaseConfig,
                    PostgresDatabaseConfig] = SqliteDatabaseConfig(kind='sqlite')
    ...
    def __post_init_post_parse__(self):
        ...
        self._callback_patterns: Dict[str, List[Sequence[HandlerPatternConfigT]]] = defaultdict(
            list)
        ...
        for filename in filenames:
            ...
            for match in re.finditer(ENV_VARIABLE_REGEX, raw_config):
                ...
                raw_config = raw_config.replace(
                    placeholder, value or default_value)
    ...
    def get_contract(self, name: str) -> ContractConfig:
        try:
            ...
            raise ConfigurationError(
                f'Contract `{name}` not found in `contracts` config section') from e
    ...
    def get_datasource(self, name: str) -> DatasourceConfigT:
        try:
            ...
            raise ConfigurationError(
                f'Datasource `{name}` not found in `datasources` config section') from e
    ...
    def get_index(self, name: str) -> IndexConfigT:
        try:
            ...
            raise ConfigurationError(
                f'Index `{name}` not found in `indexes` config section') from e
    ...
    def get_template(self, name: str) -> ResolvedIndexConfigT:
        try:
            ...
            raise ConfigurationError(
                f'Template `{name}` not found in `templates` config section') from e
    ...
    def get_hook(self, name: str) -> HookConfig:
        try:
            ...
            raise ConfigurationError(
                f'Hook `{name}` not found in `templates` config section') from e
    ...
    def initialize(self, skip_imports: bool = False) -> None:
        ...
        for index_config in self.indexes.values():
            ...
            _logger.info(
                'Loading callbacks and typeclasses of index `%s`', index_config.name)
    ...
    def _validate(self) -> None:
        ...
        if isinstance(self.database, SqliteDatabaseConfig) and self.hasura:
            raise ConfigurationError(
                'SQLite database engine is not supported by Hasura')

        # NOTE: Reserved hooks
        for name, hook_config in self.hooks.items():
            if name != hook_config.callback:
                raise ConfigurationError(
                    f'`{name}` hook name must be equal to `callback` value.')
            if name in default_hooks:
                raise ConfigurationError(
                    f'`{name}` hook name is reserved. See docs to learn more about built-in hooks.')
        ...
        plain_contracts = reduce(
            operator.add, contracts) if contracts else []  # type: ignore
        # NOTE: After pre_initialize
        duplicate_contracts = [cast(ContractConfig, item).name for item, count in Counter(
            plain_contracts).items() if count > 1]
            ...
            def get_pattern_type(pattern_items: Sequence[HandlerPatternConfigT]) -> str:
                ...
                for pattern_config in pattern_items:
                    if isinstance(pattern_config, OperationHandlerTransactionPatternConfig) and pattern_config.entrypoint:
                        module_names.append(
                            pattern_config.destination_contract_config.module_name)
    ...
    def _resolve_template(self, template_config: IndexTemplateConfig) -> None:
        _logger.info('Resolving index config `%s` from template `%s`',
                     template_config.name, template_config.template)
        ...
        with suppress(AttributeError):
            missing_value = re.search(
                r'<*>', raw_template).search(0)  # type: ignore
            raise ConfigurationError(
    ...
    def _resolve_index_links(self, index_config: IndexConfigT) -> None:
        ...
        if isinstance(index_config, OperationIndexConfig):
            if isinstance(index_config.datasource, str):
                index_config.datasource = self.get_tzkt_datasource(
            ...
            for handler_config in index_config.handlers:
                ...
                self._callback_patterns[handler_config.callback].append(
                    handler_config.pattern)
                for pattern_config in handler_config.pattern:
                    transaction_id = 0
                    if isinstance(pattern_config, OperationHandlerTransactionPatternConfig):
                        if isinstance(pattern_config.destination, str):
                            pattern_config.destination = self.get_contract(
                                pattern_config.destination)
                        if isinstance(pattern_config.source, str):
                            pattern_config.source = self.get_contract(
                                pattern_config.source)
                        ...
                        if isinstance(pattern_config.similar_to, str):
                            pattern_config.similar_to = self.get_contract(
                                pattern_config.similar_to)
                        if isinstance(pattern_config.originated_contract, str):
                            pattern_config.originated_contract = self.get_contract(
            ...
            for pattern_config in handler_config.pattern:
                if isinstance(pattern_config, OperationHandlerTransactionPatternConfig):
                    if pattern_config.entrypoint:
                        ...
                        pattern_config.initialize_storage_cls(
